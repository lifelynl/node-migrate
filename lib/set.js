...
Set.prototype._migrate = function(direction, migrationName, fn){
  ...
  if (this.migrations.length < this.pos) {
    return fn(new Error("Could not find migration " + this.pos));
  }
  ...
  function next(migration) {
    ...
    try {
      ...
      var possiblePromise = migration[direction](_next);
      if ('then' in possiblePromise) {
        possiblePromise.then(() => _next(), _next);
      }
    } catch (err) {
      fn(err);
      throw err;
